...
def key_callback_func(window, key, scancode, action, mods):
    ...
    if key == glfw.KEY_LEFT and (action == glfw.PRESS or action == glfw.REPEAT):
        rotation = rotation - 2 if rotation > -35 else -35
...
def create_transform_mat(translate_x, translate_y, translate_z, rotate_x, rotate_y, rotate_z, scale_x, scale_y, scale_z):
    ...
    transform_mat = transform_mat * \
        pyrr.matrix44.create_from_translation(
            np.array([translate_x, translate_y, translate_z]))
    transform_mat = transform_mat * \
        pyrr.matrix44.create_from_x_rotation(np.deg2rad(rotate_x))
    transform_mat = transform_mat * \
        pyrr.matrix44.create_from_y_rotation(np.deg2rad(rotate_y))
    transform_mat = transform_mat * \
        pyrr.matrix44.create_from_z_rotation(np.deg2rad(rotate_z))
    transform_mat = transform_mat * \
        pyrr.matrix44.create_from_scale(np.array([scale_x, scale_y, scale_z]))
...
def main():
    ...
    window = glfw.create_window(
        1920, 1080, "Pyopengl Texturing Cube", None, None)
...
# convert to 32bit float
    ...
    projection = pyrr.Matrix44.identity() * \
        pyrr.matrix44.create_perspective_projection(
            45.0, 1920.0 / 1080.0, 0.1, 100.0)
    ...
    while not glfw.window_should_close(window):
        ...
        camera_transform_mat = create_transform_mat(
            0.0, 0.0, 3.0, 0.0, rotation, 0.0, 1.0, 1.0, 1.0)
        perspective = projection * camera_transform_mat * view_matrix

        transformLoc = glGetUniformLocation(shader, "transform")

        for i in range(50):
            for itr in range(-60, 70, 20):
                transform_mat = create_transform_mat(itr/10+xmov+dxmov, ypos[i]+ymov+np.sin(
                    np.deg2rad(10*(i+ywav)+itr)), -i, 0.0, -i*itr/100, 0.0, 0.4, 0.4, 0.4)
                transform_mat_global = create_transform_mat(
                    0.0, 0.0, -10.0, 0.0, i*itr/100.0, 0.0, 1.0, 1.0, 1.0)
                glUniformMatrix4fv(transformLoc, 1, GL_FALSE,
                                   perspective * transform_mat_global * transform_mat)
                glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_INT, None)

            # transform_mat = create_transform_mat(-0.4+xmov+dxmov, ypos[i]+ymov+np.sin(
            #    np.deg2rad(10*(i+ywav)+60)), -i, 0.0, -i*60.0/100, 0.0, 0.5, 0.5, 0.5)
            # transform_mat_global = create_transform_mat(
            #    0.0, 0.0, 0.0, 0.0, -i*60.0/100.0, 0.0, 1.0, 1.0, 1.0)
            # glUniformMatrix4fv(transformLoc, 1, GL_FALSE,
            #                   perspective * transform_mat_global * transform_mat)
            # glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_INT, None)
